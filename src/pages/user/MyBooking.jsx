    ...
    const fetchRatings = async () => {
      ...
      try {
        const results = await Promise.allSettled(
          eligible.map((b) => api.getHotel(b.room.hotel._id)),
        );
        const map = {};
        results.forEach((r, idx) => {
          if (r.status === "fulfilled") {
            ...
            const found = hotel?.ratings?.find?.(
              (rt) => rt.booking === bookingId || rt.booking?._id === bookingId,
            );
        ...
        <ul className="mt-4 divide-y rounded-md border dark:divide-gray-800 dark:border-gray-800">
          {bookings.map((b) => {
            ...
            return (
              ...
              >
                {/* Thumbnail */}
                <div className="hidden sm:block shrink-0 overflow-hidden rounded-md border w-24 h-24 bg-gray-100 dark:bg-gray-800 dark:border-gray-700">
                  <img
                          ...
                          try {
                            ...
                            if (/already rated/i.test(msg)) {
                              try {
                                ...
                                const rt = res?.data?.hotel?.ratings?.find?.(
                                  (rt) =>
                                    rt.booking === b._id ||
                                    rt.booking?._id === b._id,
                                );
                                ...
                                console.error("Failed to refresh user rating", e);
                              }
                            }
                          } finally {
                ...
                <div className="text-right">
                  ...
                  <p
                    className={`text-xs ${["approved", "confirmed"].includes(b.status) ? "text-green-600" : b.status === "cancelled" ? "text-gray-500" : b.status === "completed" ? "text-blue-600" : "text-amber-600"}`}
                  >
                    {b.status}
                  </p>
                </div>
