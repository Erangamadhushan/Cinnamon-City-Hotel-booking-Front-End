...
const HotelDetails = () => {
  ...
  if (!hotel)
    return <p className="text-gray-600 dark:text-gray-300">Loading hotel...</p>;
    ...
    <section className="mx-auto max-w-6xl">
      ...
      <div>
        {Array.isArray(hotel.images) && hotel.images.length > 0 ? (
          <div className="mx-auto max-w-5xl">
            <div className="relative h-64 w-full overflow-hidden rounded-md border border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800 sm:h-80 md:h-96">
              {!imgError ? (
                <img
                  src={fileUrl(
                    hotel.images[Math.min(mainIdx, hotel.images.length - 1)],
                  )}
                  alt={hotel.name}
          ...
          <div className="flex h-64 w-full items-center justify-center rounded-md border border-gray-200 bg-gray-50 text-gray-400 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-500">
            No image
          </div>
        )}
      </div>
      ...
      >
        ...
        {message && (
          <p
            className={`mt-3 text-sm ${message.includes("confirmed") ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
          >
            {message}
          </p>
        )}
