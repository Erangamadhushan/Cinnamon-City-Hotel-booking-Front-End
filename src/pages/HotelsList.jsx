...
const HotelsList = () => {
  ...
  const filtered = useMemo(() => {
    ...
    if (minRating > 0)
      list = list.filter(
        (h) => typeof h.rating === "number" && h.rating >= minRating,
      );
    switch (sort) {
      case "name":
    ...
    <section>
      ...
      <div className="mt-4 flex flex-wrap gap-2 items-center">
        <input
          ...
          className="border px-3 py-2 rounded-md dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100 placeholder-gray-400"
        />
        <input
          value={city}
        ...
        <select
          ...
          className="border px-3 py-2 rounded-md dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100"
        >
        ...
        <label className="ml-1 text-sm text-gray-700 dark:text-gray-300">
          Min rating
        </label>
        <input
          type="number"
          ...
          className="w-24 border px-3 py-2 rounded-md dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100"
        />
        <button onClick={applyAndSearch} className="btn btn-primary">
          Search
        </button>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        {!loading && filtered.length === 0 && (
          <p className="text-gray-600 dark:text-gray-300">No hotels found.</p>
        )}
      </div>
      {/* Pagination */}
      {!loading && filtered.length > 0 && (
